...
public sealed class GlobalExceptionHandler : IExceptionHandler
{
    ...
    private static (int StatusCode, string Message) Map(Exception exception) => exception switch
    {
        ...
        // FluentValidation (ValidationBehavior στο MediatR pipeline) - πριν από το γενικό
        // ArgumentException, γιατί θέλουμε τα μηνύματα ανά πεδίο, όχι το γενικό Message.
        ValidationException validationEx => (StatusCodes.Status400BadRequest, BuildValidationMessage(validationEx)),
        ...
        // Γενικό "σπάσιμο κανόνα" (business rule violation) που δεν έχει δικό του
        // domain exception type - π.χ. λάθος κατάσταση/sequence, όχι λάθος input.
        InvalidOperationException => (StatusCodes.Status400BadRequest, exception.Message),
        IpLookupException => (StatusCodes.Status502BadGateway, exception.Message),
        _ => (StatusCodes.Status500InternalServerError, "An unexpected error occurred.")
    };

    // Ίδιο { error } shape με όλα τα άλλα - απλά το message είναι όλα τα field errors μαζί.
